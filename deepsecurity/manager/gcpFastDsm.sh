...
yum -y install docker-engine jq
while [ $? -ne 0 ]; do !!; done
...
echo "AddressAndPortsScreen.ManagerAddress=$(curl http://169.254.169.254/0.1/meta-data/network | jq -r .networkInterface[0].ip)" >> dsm.props
...
./Manager-Linux.sh -q -console -varfile dsm.props || exit -1
echo "$(date) -- manager install complete"
chkconfig --add dsm_s
systemctl enable dsm_s 
chkconfig dsm_s on
...
if [ -z ${name} ]; then name=$(curl http://169.254.169.254/0.1/meta-data/network | jq -r .networkInterface[0].accessConfiguration[0].externalIp); fi
